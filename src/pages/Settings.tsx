    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="glass-card rounded-xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <Input type="number" step="0.1" min="0" max="100" value={taxSettings.federalRate} onChange={(e) => setTaxSettings((s) => ({ ...s, federalRate: parseFloat(e.target.value) || 0 }))} />
          </div>
          <div>
            <Label className="text-xs text-muted-foreground mb-1.5 block">State Tax Rate (%)</Label>
            <Input type="number" step="0.1" min="0" max="100" value={taxSettings.stateRate} onChange={(e) => setTaxSettings((s) => ({ ...s, stateRate: parseFloat(e.target.value) || 0 }))} />
          </div>
          <div>
            <Label className="text-xs text-muted-foreground mb-1.5 block">B&O Tax Rate (%)</Label>
            <Input type="number" step="0.1" min="0" max="100" value={taxSettings.bnoRate} onChange={(e) => setTaxSettings((s) => ({ ...s, bnoRate: parseFloat(e.target.value) || 0 }))} />
          </div>
        </div>
      </div>
      ...
      <div className="glass-card rounded-xl p-6 space-y-5">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={handleAddCompany} className="gap-1.5">
            <Plus className="h-4 w-4" /> Add Company
          </Button>
        </div>

        {companies.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-6">No companies added yet. Click "Add Company" to get started.</p>
        )}
            ...
            <div key={company.id} className="border border-border rounded-lg p-4 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-[1fr_140px_auto] gap-3 items-end">
                <div>
                  ...
                  <Input value={company.name} onChange={(e) => updateCompany(company.id, { name: e.target.value })} placeholder="Company name" />
                </div>
                <div>
                ...
                <Button variant="ghost" size="icon" className="h-10 w-10 text-muted-foreground hover:text-destructive" onClick={() => setDeleteCompanyId(company.id)}>
              ...
              {company.name && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>{company.name} — {company.companyType}{!company.includeInTax && " (excluded from tax calculations)"}</span>
                </div>
              )}
            </div>
          ))}
      ...
      <AlertDialog open={!!deleteCompanyId} onOpenChange={(open) => !open && setDeleteCompanyId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently remove this company from your settings. This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

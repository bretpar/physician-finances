...
async function syncCatchupMirror(args: {
  ...
  if (isBusiness && fedW > 0) {
    // Derive the IRS estimated-tax bucket from period_end.
    // Must match `getCurrentQuarter` in src/lib/quarters.ts (IRS estimated-
    // tax periods) so the Dashboard tracker and Tax Overview attribute the
    // same income/payment to the same quarter.

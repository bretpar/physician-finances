...
// ─── Standalone section components (drop into any collapsed container) ───

export function TaxableIncomeMath({ data }: { data: TaxBreakdownResult }) {
...
export function BracketBreakdownMath({ data }: { data: TaxBreakdownResult }) {
  const filingLabel =
  ...
  return (
    <div className="space-y-0">
      <p className="text-xs text-muted-foreground pb-2">
        Employee-side FICA shown for transparency. These amounts are
      ...
      <p className="text-[11px] text-muted-foreground pt-2">
        Shown separately from federal and state income-tax withholding.
      </p>
    </div>
  );
}
...
// Default export kept for back-compat (single accordion card)
      ...
      <CardContent className="pt-0">
        <Accordion type="multiple" className="w-full">
          <AccordionItem value="taxable">
            <AccordionTrigger className="text-sm">Taxable income calculation</AccordionTrigger>
            <AccordionContent><TaxableIncomeMath data={data} /></AccordionContent>
          </AccordionItem>
          <AccordionItem value="brackets">
            <AccordionTrigger className="text-sm">Tax bracket breakdown</AccordionTrigger>
            <AccordionContent><BracketBreakdownMath data={data} /></AccordionContent>
          </AccordionItem>
          {data.totalW2Income > 0 && (
            <AccordionItem value="w2-payroll">
              <AccordionTrigger className="text-sm">W-2 payroll tax (Social Security &amp; Medicare)</AccordionTrigger>
              <AccordionContent><W2PayrollTaxMath data={data} /></AccordionContent>
            </AccordionItem>
          )}
          {data.seTax.total > 0 && (
            <AccordionItem value="se">
              <AccordionTrigger className="text-sm">Self-employment tax breakdown</AccordionTrigger>
              <AccordionContent><SETaxMath data={data} /></AccordionContent>
            </AccordionItem>
          )}
          {(data.totalShortTermGains > 0 || data.totalLongTermGains > 0) && (
            <AccordionItem value="capgains">
              <AccordionTrigger className="text-sm">Capital gains breakdown</AccordionTrigger>
              <AccordionContent><CapitalGainsMath data={data} /></AccordionContent>
            </AccordionItem>
          )}
        </Accordion>
      </CardContent>

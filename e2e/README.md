...
- `PLAYWRIGHT_IGNORE_HTTPS_ERRORS` or `E2E_IGNORE_HTTPS_ERRORS` — `1` or
  `true` to force `ignoreHTTPSErrors: true`, `0` or `false` to force it off.
  When unset, defaults to `true` in CI (where containers often lack an
  up-to-date root CA bundle and would otherwise fail with
